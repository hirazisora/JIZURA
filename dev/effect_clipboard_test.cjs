...
const pasted=await page.evaluate(()=>JSON.parse(JSON.stringify(J.ui.plan.cuts.find(c=>c.line===1&&c.part===0))));assert.deepEqual(pasted.area,target.area);assert.equal(pasted.text,target.text);assert.equal(pasted.start,target.start);assert.equal(pasted.end,target.end);assert.equal(pasted.enter,'wipe');assert.equal(pasted.opacity,43);assert.deepEqual(pasted.fonts,payload.details.fonts);assert.deepEqual(pasted.palette,payload.details.palette);assert.equal(pasted.params.font,payload.details.fontParams.find(e=>e.path.join('.')==='font').font);
