...
const payload=await page.evaluate(()=>JSON.parse(window.testClipboard));assert.equal(payload.kind,'lyrics');assert.equal(payload.details.area,undefined);assert.equal(payload.details.text,undefined);assert.equal(payload.details.params,undefined);
...
const pasted=await page.evaluate(()=>JSON.parse(JSON.stringify(J.ui.plan.cuts.find(c=>c.line===1&&c.part===0))));assert.deepEqual(pasted.area,target.area);assert.equal(pasted.text,target.text);assert.equal(pasted.start,target.start);assert.equal(pasted.end,target.end);assert.equal(pasted.enter,'wipe');assert.equal(pasted.opacity,43);
