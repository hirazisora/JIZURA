...
`｜　　　｜`（全角の縦線で空白を囲む）を入力すると、文字を描かず演出だけを表示する歌詞行を作れます。半角スペースにも対応し、行の開始時刻や境界リンクは通常の歌詞と同様に設定できます。

歌詞の表示領域は画面外へ移動・拡大でき、幅・高さは最大400％です。詳細 → 背景の「歌詞の背景演出も適用」（初期値ON）で、歌詞の背景グラフィックを表示するか切り替えられます。背景素材がある場合も、その上へ演出を重ねます。

### 歌詞を残しながら重なりを避ける

歌詞を `{-` と `-}` で囲むと、`{ … }` と同じように最後まで順に残しつつ、グループ内の歌詞の表示領域が重ならないように配置します。
...
**詳細 → 手法 → 歌詞同士の重なり回避の強度**で調整できます。初期値は1、0では通常の重ね表示です。自動配置がオフでも適用し、必要に応じて縦横比を保って縮小します。配置は再生・シークで変わりません。記号を文字として表示する場合は `\{`・`\}` のようにエスケープします。

### 演出のコピー・ペースト

各カットの操作欄・タイムライン・プレビュー枠のコピー／ペーストアイコンで演出を移せます。歌詞同士、または前景・背景同士で利用できます。貼り付け先の本文・素材・開始時刻・表示エリア／配置は維持し、レイアウトの詳細パラメータ・描画用乱数・スタイル／強度・カットに付随するエフェクトイベントも複製します。内部に保持する歌詞の文字列のみ貼り付け先の本文に合わせて再構成します。貼り付け先の長さに収まらない登場・退場時間やイベントは、そのカットの長さに合わせて制限されます。貼り付けはアンドゥ／やり直しに対応します。クリップボードが使えない環境では、同じページ内でのコピー・ペーストを利用できます。


### 設定のみの書き出し・読み込み

「プロジェクト → 設定のみ書き出し」で、テーマ・スタイル・演出候補・演出の強さ・色・フォント（取り込んだフォントファイルを含む）・前景／背景の共通演出設定・出力サイズ／画質などを `.jizuraichi` に保存できます。歌詞・画像・動画・音声・各カットの設定は含みません。

「設定のみ読み込み」は、設定用ファイルと通常のプロジェクトファイルの両方に対応します。現在の歌詞・曲名・音声・素材・各カットの個別設定・順番・開始時刻・リンク・動画の長さを保持し、共通設定を適用します。曲に紐づくBPM／同期設定や歌詞言語、素材のループ／ランダム順は変更しません。既存の歌詞カットの分割を維持し、読み込んだ「カットの細かさ」は新しい行に使われます。このスライダーを操作すると、既存行も再分割されます。読み込みはアンドゥできます。


### 歌詞入力補助と演出コピー

歌詞欄の下に「カット」「改行」「強調」「抑制」「1シーン」「重ねず1シーン」「空の歌詞」の入力補助ボタンがあります。未選択ならカーソル位置に記法を挿入し、選択中なら対象を囲みます。カットと改行は選択範囲の前後に挿入します。「改行」は1カット内の改行記法 `\n`、「空の歌詞」は全角の縦線と全角スペース4つです。シーン記法は独立した行になるよう改行も補います。

歌詞の演出コピーには、使用フォントと配色も含まれます。貼り付け先の歌詞・表示エリアは保持されます。注釈記法 `歌詞|ruby or note` の小さな文字は、対応している演出のみで表示されます。


入力補助ボタンの各クリックは独立してアンドゥ／リドゥできます。歌詞欄内でも Ctrl+Z、Ctrl+Shift+Z（または Ctrl+Y）が利用できます。「空の歌詞」は、未選択なら全角スペース4つ、選択中なら元の半角／全角文字を対応するスペースに置き換えます。挿入後は改行し、選択を解除して次の行へカーソルを移動します。複数行の選択では各行を空の歌詞にします。


### 前景・背景のカット別合成

前景・背景の「行とカット」で、各カットに合成方法（通常・乗算・スクリーン・オーバーレイ）と不透明度（0〜100％）を指定できます。従来の前景全体の設定欄はカット内へ移動しました。既存プロジェクトの前景設定は引き継がれます。背景はベースの背景色に対して合成します。設定はプレビュー・動画書き出し・プロジェクト保存・演出コピペ・アンドゥに対応します。


### 手法・演出のプレビュー

詳細 → 手法、および詳細 → 背景／前景の各演出名の横にある ▶ ボタンで、名称と再生画面のプレビューモーダルを開けます。歌詞は `プレビュー|ルビ`、画像は付属の赤・青の長方形画像で、約6秒のプレビューを繰り返し再生します。編集中の歌詞・素材・チェック状態は変更しません。× または Esc で閉じるとプレビューが停止します。


### OFFにして再抽選

再抽選ボタンを一回転の矢印に変更し、隣に赤いマイナス付きの「OFFにして再抽選」を追加しました。行とカット・タイムライン・プレビュー枠から操作できます。歌詞では現在のレイアウト、登場・保持・退場、装飾、加工、背景、カメラ、画面効果、繋ぎを詳細の候補からOFFにします。前景・背景ではそのレイヤーの手法・登場・退場をOFFにします。その後、固定された演出指定を解除して再抽選します。歌詞は従来と同様に行単位で再抽選し、本文・表示エリアは保持します。OFF設定と再抽選はまとめてアンドゥできます。


背景タブの「カット挿入」最上部にある「一括挿入する項目」で、画像・動画／前景のコピー（素材のみ）／前景のコピー（演出を含む）／歌詞のコピーを選択できます。初期値は画像・動画のみONです。複数選択すると項目をランダムに選び、歌詞に合わせて一括挿入します。コピー項目はアップロード素材なしでも使用できます。画像・動画は既存の並び順・ランダム順・ループ設定に従い、ループOFFで使い切った後はコピー項目から選びます。
