...
function disableRerollButton(layer,index,part=null){
  ...
  button.title=J.mediaLabel('OFFにして再抽選','Disable current effects and randomize');button.setAttribute('aria-label',button.title);button.innerHTML=ICON.disableReroll;
...
function drawItemFrames() {
  ...
  const L=J.mediaLabel,labels={copy:L('演出をコピー','Copy effects'),paste:L('演出をペースト','Paste effects'),dice:L('再抽選','Randomize'),disableReroll:L('OFFにして再抽選','Disable current effects and randomize'),lock:L('ロック','Lock'),area:L('表示範囲','Display area'),details:L('詳細編集','Edit details'),remove:L('削除','Delete'),frontmost:L('最前に表示','Frontmost')};
...
function drawTimelineLinks() {
  ...
  const action = (layer, cut, index, locked) => {
    ...
    const y = canvas.offsetTop + 11;
    const actions = [['dice', false, layer === 'lyrics' ? 'この行を再抽選' : 'このカットを再抽選', ICON.dice], ['disableReroll',false,J.mediaLabel('OFFにして再抽選','Disable current effects and randomize'),ICON.disableReroll], ['lock', locked, layer === 'lyrics' ? 'この行の構成をロック' : 'このカットをロック', ICON.lock]];
...
function bind() {
  ...
  $('timelineLegend').innerHTML = [
    ...
    [ICON.disableReroll,'OFFにして再抽選','Disable current effects and randomize',''],
    [ICON.lock,'ロック','Lock',''],
