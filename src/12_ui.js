...
function drawTimelineLinks() {
  ...
  const action = (layer, cut, index, locked) => {
    ...
    const y = canvas.offsetTop + 11;
    const actions = [['dice', false, layer === 'lyrics' ? 'この行を再抽選' : 'このカットを再抽選', ICON.dice], ['disableReroll',false,J.mediaLabel('この演出をOFFにして再抽選','Disable current effects and randomize'),ICON.disableReroll], ['lock', locked, layer === 'lyrics' ? 'この行の構成をロック' : 'このカットをロック', ICON.lock]];
